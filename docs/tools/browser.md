...
- Key collaborators:
  ...
  - `packages/coding-agent/src/tools/browser/cmux/tui-client.ts` — read-only GUI/TUI protocol detection plus explicit browser-only codecs for release `cmux.protocol/1` at `manaflow-ai/cmux@a2b3c10f119324c9dbfecc29881d30ff54404a84` and current `cmux.protocol/2` at `manaflow-ai/cmux@ea093bb294fbc4a7da628a4451c0bc721d5e04e9`, including reconnect, versioned stream endings, protocol/2 creation-receipt recovery and attachment leases, and browser streams.
  - `packages/coding-agent/src/tools/browser/cmux/tui-tab.ts` — typed cmux-tui browser resource facade for navigation, frames/screenshots, resizing, keyboard/text, and pointer-frame-guarded input.
...
| `app` | `{ path?: string; cdp_url?: string; relay?: boolean; cmux?: boolean; surface?: string; args?: string[]; target?: string }` | No | Selects browser kind. Explicit `app.cdp_url` wins, then `app.path`; `app.cmux: true` or `app.surface` explicitly selects `CMUX_SOCKET_PATH`, while `app.cmux: false` suppresses cmux for this call. For cmux-tui, `surface` must be the exact stable `browser_…` resource ID to borrow; omitting it creates an OMP-owned browser that closes with the managed tab. `app.relay: true` opts into the OMP Browser Relay; `app.relay: false` suppresses relay settings for this call. With no explicit app kind, `browser.relay` (overridden by `PI_BROWSER_RELAY`) precedes `browser.cdpUrl`, then cmux when available, then `browser.headless`. `browser.relayUrl` defaults to `http://127.0.0.1:9224`. `args` apply only to spawned `app.path`; `target` selects an attached/spawned/relay page by URL/title substring. |
...
2. `open` resolves browser kind with `resolveBrowserKind()`:
   ...
   - otherwise, `resolveCmuxKind()` → `{ kind: "cmux", backend: "auto", socketPath, password?, surface? }` when `CMUX_SOCKET_PATH` is set and cmux is enabled (`browser.cmux`, overridable by `PI_BROWSER_CMUX`).
...
4. `open` acquires a browser handle through `acquireBrowser()` (`packages/coding-agent/src/tools/browser/registry.ts`):
   ...
   - `cmux` sends only raw `identify` to detect the socket protocol. A GUI cmux-socket v2 response preserves the existing `CmuxSocketClient` route. A cmux-tui response selects raw v10/`cmux.protocol/1` or raw v12/`cmux.protocol/2` before the typed client sends `machine.list`/`session.list` and retains those stable route IDs. Each connection accepts only its selected envelope version; TUI handles receive a read-only liveness check before reuse and reconnect reads only when the stable machine/session route is unchanged. Other versions and mixed envelopes fail closed without a mutation-based fallback or silent downgrade.
5. `open` acquires a tab through `acquireTab()` (`packages/coding-agent/src/tools/browser/tab-supervisor.ts`):
   ...
   - cmux GUI opens or attaches to the existing surface UUID flow. cmux-tui accepts a stable `browser_…` ID in `app.surface`, or creates a browser tab with `tab.create_browser` and retains the returned `ws_…`/`browser_…` identities. A supplied URL is loaded before `open` returns, and the TUI protocol supports only `wait_until: "load"`; other wait policies fail before browser creation. Each TUI tab owns a browser attachment stream, requires an initial frame before `open` returns, recovers a gap with a fresh initial snapshot, invalidates pointer authority on navigation/resize/state changes, and marks a `closed` stream or missing browser resource dead instead of reattaching it. Later URL/navigation/resize completion follows browser state even when `frames_stalled` is true; stale pixels are discarded and the next screenshot or pointer action waits for its own fresh frame. Browser resources are closed only when OMP created them.
...
- **Browser kind**
  ...
  - **GUI cmux surface (`browser.cmux`)**: a GUI cmux-socket v2 endpoint drives the existing WKWebView surface JSON-RPC backend. No Bun worker and no stealth patches; `open` opens a split (owning that surface), `run` executes via `runCmuxCode()`, and `close` issues `surface.close` only for surfaces it owns (leaving the workspace's last surface open).
  - **cmux-tui browser resource (`browser.cmux`)**: raw-v10 and raw-v12 cmux-tui endpoints are routed explicitly to their public `cmux.protocol/1` and `/2` resource APIs. Both use immutable machine/session/workspace/browser IDs, typed load-complete navigation and input mutations, streamed PNG/JPEG screenshots, pointer-frame guards, abort/deadline propagation, read reconnects, and leak-free stream-gap reattachment. Protocol/1 uses correlation resolution for uncertain idempotent creation and connection-owned viewer resize/release; protocol/2 adds attachment-lease-bound viewer operations. `app.surface` must be a full `browser_…` ID. Selector/DOM/evaluation helpers fail explicitly because the public resource protocols expose frames and input, not page JavaScript or DOM handles.
